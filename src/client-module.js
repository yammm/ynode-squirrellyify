 ...
 *
 * @typedef {object} ClientDeclarationType
 * @property {string} name Named type export used as the renderer's data parameter.
 * @property {string} from Module specifier resolved by TypeScript from the generated declaration.
 *
 * @typedef {Record<string, ClientDeclarationType>} ClientDeclarationTypes
 *
 ...
 * @property {ClientDeclarationTypes} [declarationTypes] Optional template-to-import mapping for typed renderer data parameters.
...
/**
 * Compares strings by Unicode code unit so generated artifact ordering does
...
/**
 * Validates renderer declaration mappings and deduplicates identical imports.
 *
 * @param {ClientDeclarationTypes|undefined} value Configured mappings.
 * @param {string[]} templateNames Known renderer names.
 * @param {string} [label] Configuration path used in validation errors.
 * @returns {{importedTypes: Map<string, ClientDeclarationType>, rendererTypes: Map<string, string>}} Validated imports and renderer-to-import keys.
 */
function resolveDeclarationTypes(value, templateNames, label) {
    if (value === undefined) {
        return { importedTypes: new Map(), rendererTypes: new Map() };
    }
    assertPlainObject(value, label);

    const knownTemplates = new Set(templateNames);
    const importedTypes = new Map();
    const rendererTypes = new Map();
    for (const templateName of Object.keys(value).sort()) {
        if (!knownTemplates.has(templateName)) {
            throw new TypeError(`${label} contains unknown template: ${templateName}`);
        }
        const entryLabel = `${label}.${templateName}`;
        const entry = value[templateName];
        assertPlainObject(entry, entryLabel);
        const unknownKeys = Object.keys(entry)
            .filter((key) => key !== "name" && key !== "from")
            .sort();
        if (unknownKeys.length > 0) {
            throw new TypeError(`${entryLabel} contains unknown option: ${unknownKeys[0]}`);
        }
        assertRuntimeName(entry.name, entryLabel, { identifier: true });
        if (
            typeof entry.from !== "string" ||
            entry.from.length === 0 ||
            entry.from.trim() !== entry.from ||
            /\p{Cc}/u.test(entry.from)
        ) {
            throw new TypeError(
                `${entryLabel}.from must be a non-empty module specifier without surrounding whitespace or control characters.`,
            );
        }

        const importKey = JSON.stringify([entry.from, entry.name]);
        importedTypes.set(importKey, { name: entry.name, from: entry.from });
        rendererTypes.set(templateName, importKey);
    }
    return { importedTypes, rendererTypes };
}

/**
 * Generates a declaration with collision-safe aliases for imported data types.
 *
 * @param {string[]} templateNames Ordered renderer names.
 * @param {boolean} isAsync Whether renderers return promises.
 * @param {ClientDeclarationTypes|undefined} declarationTypes Renderer type mappings.
 * @param {string} [label] Configuration path used in validation errors.
 * @returns {string} Generated TypeScript declaration source.
 */
function declarationSource(templateNames, isAsync, declarationTypes, label) {
    const returnType = isAsync ? "Promise<string>" : "string";
    const resolvedTypes = resolveDeclarationTypes(declarationTypes, templateNames, label);
    const sortedTypes = [...resolvedTypes.importedTypes]
        .sort(
            ([, left], [, right]) =>
                compareStrings(left.from, right.from) || compareStrings(left.name, right.name),
        )
        .map(([key, value], index) => ({
            ...value,
            alias: `__SquirrellyType${index}`,
            key,
        }));
    const typeAliases = new Map(sortedTypes.map(({ alias, key }) => [key, alias]));
    const importsBySource = new Map();
    for (const importedType of sortedTypes) {
        const types = importsBySource.get(importedType.from) ?? [];
        types.push(importedType);
        importsBySource.set(importedType.from, types);
    }
    const imports = [...importsBySource]
        .sort(([left], [right]) => compareStrings(left, right))
        .map(
            ([moduleSpecifier, types]) =>
                `import type { ${types.map(({ alias, name }) => `${name} as ${alias}`).join(", ")} } from ${JSON.stringify(moduleSpecifier)};`,
        )
        .join("\n");
    const renderers = templateNames
        .map(
            (name) =>
                `    ${name}(data: ${typeAliases.get(resolvedTypes.rendererTypes.get(name)) ?? "Record<string, unknown>"}): ${returnType};`,
        )
        .join("\n");
    return `// Generated by @ynode/squirrellyify. Do not edit.\n${imports ? `${imports}\n\n` : ""}export declare const render: Readonly<{\n${renderers}\n}>;\n`;
}

async function compileClientModuleArtifacts(options = {}, declarationTypes, declarationTypesLabel) {
...
export async function buildClientModules(options = {}) {
    ...
    const modules = moduleNames.map((name) => {
        ...
        const module = {
            ...
            output,
            declaration,
            declarationTypes: input.declarationTypes,
            sourceMap,
            compile: {
    ...
    for (const module of modules) {
        const compiled = await compileClientModuleArtifacts(
            module.compile,
            module.declarationTypes,
            `modules.${module.name}.declarationTypes`,
        );
        const artifacts = [];
