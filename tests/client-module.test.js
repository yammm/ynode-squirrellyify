...
test("emits imported renderer data types with deterministic generic fallbacks", async (t) => {
    const dir = await createTempDir(t);
    const [built] = await buildClientModules({
        cwd: dir,
        modules: {
            history: {
                ...
                templates: {
                    summary: "{{ it.total }}",
                    tableBody: "{{ it.rows.length }}",
                    untyped: "{{ it.value }}",
                },
                declarationTypes: {
                    summary: { name: "SummaryViewData", from: "../../types/history.js" },
                    tableBody: { name: "HistoryRowsViewData", from: "../../types/history.js" },
                },
            },
        },
    });
    ...
    assert.equal(
        await fs.readFile(built.declaration, "utf-8"),
        `// Generated by @ynode/squirrellyify. Do not edit.
import type { HistoryRowsViewData as __SquirrellyType0, SummaryViewData as __SquirrellyType1 } from "../../types/history.js";

export declare const render: Readonly<{
    summary(data: __SquirrellyType1): string;
    tableBody(data: __SquirrellyType0): string;
    untyped(data: Record<string, unknown>): string;
}>;
`,
    );
});

test("aliases declaration imports away from generated names and global utility types", async (t) => {
    const dir = await createTempDir(t);
    const [built] = await buildClientModules({
        cwd: dir,
        modules: {
            aliases: {
                output: "public/aliases.js",
                config: { async: true },
                templates: {
                    keyword: "ok",
                    promise: "ok",
                ...
                declarationTypes: {
                    keyword: { name: "default", from: "../../types/default.js" },
                    promise: { name: "Promise", from: "../../types/globals.js" },
                    record: { name: "Record", from: "../../types/globals.js" },
                    render: { name: "render", from: "../../types/render.js" },
                },
            },
        },
    });
    const declaration = await fs.readFile(built.declaration, "utf-8");

    assert.match(declaration, /default as __SquirrellyType0/u);
    assert.match(declaration, /Promise as __SquirrellyType1/u);
    assert.match(declaration, /Record as __SquirrellyType2/u);
    assert.match(declaration, /render as __SquirrellyType3/u);
    assert.match(declaration, /keyword\(data: __SquirrellyType0\): Promise<string>/u);
    assert.match(declaration, /promise\(data: __SquirrellyType1\): Promise<string>/u);
    assert.match(declaration, /record\(data: __SquirrellyType2\): Promise<string>/u);
    assert.match(declaration, /render\(data: __SquirrellyType3\): Promise<string>/u);
});
...
test("rejects unsupported or incomplete client module definitions", async () => {
            ...
            buildClientModules({
                modules: {
                    page: {
                        ...
                        templates: { page: "ok" },
                        declarationTypes: {
                            missing: { name: "PageData", from: "./types.js" },
                        },
                    },
                },
            }),
        /declarationTypes contains unknown template: missing/,
    );
    await assert.rejects(
        () =>
            buildClientModules({
                modules: {
                    page: {
                        ...
                        templates: { page: "ok" },
                        declarationTypes: {
                            page: { name: "Page<Data>", from: "./types.js" },
                        },
                    },
                },
            }),
    ...
    for (const from of ["", " ./types.js", "./types.js\n"]) {
        await assert.rejects(
            () =>
                buildClientModules({
                    modules: {
                        page: {
                            output: "page.js",
                            templates: { page: "ok" },
                            declarationTypes: { page: { name: "PageData", from } },
                        },
                    },
                }),
            /must be a non-empty module specifier/,
        );
    }
});
